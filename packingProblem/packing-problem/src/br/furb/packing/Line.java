...
public class Line {
	...
	public TouchingEdgeVertex compare(Line line) {
		if (start.equals(line.start)) {
			return TouchingEdgeVertex.START_START;
		}
		if (start.equals(line.end)) {
			return TouchingEdgeVertex.START_END;
		}
		if (end.equals(line.start)) {
			return TouchingEdgeVertex.END_START;
		}
		if (end.equals(line.end)) {
			return TouchingEdgeVertex.END_END;
		}
		throw new IllegalStateException("Nenhum ponto é igual.");
	}

	public boolean intersectsLine(Point p1, Point p2) {
		return linesIntersect(p1, p2, start, end);
	}
	 ...
	 * 
	 * @param px
	 *            the X coordinate of the specified point to be compared with this <code>Line2D</code>
	 * @param py
	 *            the Y coordinate of the specified point to be compared with this <code>Line2D</code>
	 * @return an integer that indicates the position of the specified coordinates with respect to this <code>Line2D</code>
	 * @see #relativeCCW(double, double, double, double, double, double)
	 * @since 1.2
	 */
	public int relativeCCW(double px, double py) {
		return relativeCCW(start.x, start.y, end.x, end.y, px, py);
	}
	...
	public static boolean linesIntersect(Point p1, Point p2, Point p3, Point p4) {
		return ((relativeCCW(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y) * //
				relativeCCW(p1.x, p1.y, p2.x, p2.y, p4.x, p4.y) <= 0) && //
		(relativeCCW(p3.x, p3.y, p4.x, p4.y, p1.x, p1.y) * //
				relativeCCW(p3.x, p3.y, p4.x, p4.y, p2.x, p2.y) <= 0));
	}
	...
	public static int relativeCCW(double x1, double y1, double x2, double y2, double px, double py) {
		...
		px -= x1;
		py -= y1;
		if (Math.abs(x2) < Transform.THRESHOLD2) {
			x2 = 0;
		}
		if (Math.abs(y2) < Transform.THRESHOLD2) {
			y2 = 0;
		}
		if (Math.abs(px) < Transform.THRESHOLD2) {
			px = 0;
		}
		if (Math.abs(py) < Transform.THRESHOLD2) {
			py = 0;
		}
		double ccw = px * y2 - py * x2;
		// equivalente a ccw == 0.0
		if (Math.abs(ccw) < Transform.THRESHOLD) {
	...
	public double ptSegDist(double px, double py) {
		return ptSegDist(start.x, start.y, end.x, end.y, px, py);
	}
	...
	public static double ptSegDistSq(double x1, double y1, double x2, double y2, double px, double py) {
		...
		// customização
		if (Math.abs(lenSq) <= Transform.THRESHOLD) {
			lenSq = 0;
		}
		return lenSq;
	}
